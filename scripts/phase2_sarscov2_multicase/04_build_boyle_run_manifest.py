...
INPUT = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_2025_prjna1295507_prjna1295507_ena_read_run.tsv"

OUT_MANIFEST = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_run_manifest.csv"
OUT_SUMMARY = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_run_manifest_summary.csv"
OUT_SAMPLE_TEMPLATE = ROOT / "data" / "interim" / "phase2_sarscov2_multicase" / "boyle_sample_day_mapping_template.csv"
...
def main():
    if not INPUT.exists():
        raise FileNotFoundError(
            f"Missing Boyle ENA run table: {INPUT}. "
            "Run scripts/phase2_sarscov2_multicase/03_audit_phase2_accessions.py first."
        )
    ...
    summary = pd.DataFrame([{
        ...
        "next_required": "recover sample-day mapping and patient/case grouping before HRSM extraction",
    }])
    OUT_SUMMARY.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(OUT_SUMMARY, index=False)

    # Template to manually or programmatically fill from Supplemental Table S1.
    sample_template = out[[
        "case_id",
        "sample_title",
        "sample_accession",
