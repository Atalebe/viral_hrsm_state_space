...
RUN_MANIFEST = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_run_manifest.csv"

OUT_BIOSAMPLE = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_biosample_metadata.csv"
OUT_JOINED = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_run_manifest_with_biosample_metadata.csv"
OUT_SUMMARY = ROOT / "metadata" / "phase2_sarscov2_multicase" / "boyle_biosample_metadata_summary.csv"

EUTILS = "https://eutils.ncbi.nlm.nih.gov/entrez/eutils"

def fetch_json(url):
    req = urllib.request.Request(url, headers={"User-Agent": "viral-hrsm-biosample-audit"})
...
def parse_biosample_xml(accession, uid, xml_text):
    ...
    try:
        ...
        # Flatten selected likely-useful attributes.
        preferred = [
            "collection_date",
            "geo_loc_name",
            "host",
...
def main():
    if not RUN_MANIFEST.exists():
        raise FileNotFoundError(f"Missing Boyle run manifest: {RUN_MANIFEST}")
    ...
    # Summarize useful mapping fields.
    ...
    summary = pd.DataFrame([{
        ...
        "next_required": "inspect biosample attributes for patient/time grouping; if collection_date absent, recover Supplemental Table S1 manually",
    }])
    ...
    print(joined[preview_cols].head(80).to_string(index=False))
