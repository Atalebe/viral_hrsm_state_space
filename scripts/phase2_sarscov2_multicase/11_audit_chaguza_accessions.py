...
ARTICLE_URLS = [
    "https://pmc.ncbi.nlm.nih.gov/articles/PMC9906997/",
]

OUT_DIR = ROOT / "metadata" / "phase2_sarscov2_multicase"
OUT_ARTICLE_SUMMARY = OUT_DIR / "chaguza_article_accession_summary.csv"
OUT_LINKS = OUT_DIR / "chaguza_article_links.csv"
OUT_ACCESSIONS = OUT_DIR / "chaguza_accession_candidates.csv"
OUT_ENA_AUDIT = OUT_DIR / "chaguza_ena_accession_audit.csv"
...
def classify_link(text, href):
    ...
    if href.lower().endswith((".xlsx", ".xls", ".csv", ".tsv", ".txt", ".zip", ".pdf")):
        tags.append("downloadable_candidate")
    if "sra" in hay or "ena" in hay or "ncbi" in hay or "prjna" in hay or "err" in hay or "srr" in hay or "samn" in hay:
...
def main():
    ...
    accession_pattern = re.compile(r"\b(PRJNA\d+|PRJEB\d+|SRP\d+|SRR\d+|ERR\d+|SAMN\d+|ERS\d+|EPI_ISL_\d+)\b")
    ...
    for url in ARTICLE_URLS:
        ...
        accessions = sorted(set(accession_pattern.findall(text)))
        ...
        summary_rows.append({
            ...
            "accession_candidates": "|".join(accessions[:100]),
            "mentions_data_availability": "data availability" in text.lower(),
            "mentions_supplement": "supplement" in text.lower(),
        })
    ...
    # Query only project/study-style accessions first. Individual SRR/ERR can be too many.
    project_accessions = accession_df[
        accession_df["accession"].astype(str).str.match(r"^(PRJNA|PRJEB|SRP)")
    ]["accession"].drop_duplicates().tolist()

    for acc in project_accessions:
        print({"status": "fetching_ena", "accession": acc})
        raw_path = OUT_DIR / f"chaguza_{acc.lower()}_ena_read_run.tsv"
        try:
            tsv = fetch_ena(acc)
