...
\section*{Entry 038: Weigang primary swab raw-download plan created}

\textbf{Date:} \today

\textbf{Purpose:} To create a raw-data download plan for the mapped Weigang primary swab trajectory without downloading raw files.

\textbf{Script added:}
\begin{itemize}
    \item \texttt{scripts/phase2\_sarscov2\_multicase/18\_plan\_weigang\_primary\_swab\_downloads.py}
\end{itemize}

\textbf{Interpretation:} The plan restricts raw download scope to the nine primary swab timepoints. Isolate controls are excluded from the initial raw-feature extraction branch. Submitted FASTQ links are preferred where available.

\textbf{Failure guard:} Raw files remain planned, not downloaded. The extraction protocol must be frozen before any large files are pulled into the workspace.

\section*{Entry 039: Weigang raw-feature protocol frozen before download}

\textbf{Date:} \today

\textbf{Purpose:} To freeze the raw-feature extraction protocol for the mapped Weigang primary swab trajectory before downloading FASTQ files.

\textbf{Files added:}
\begin{itemize}
    \item \texttt{docs/phase2\_sarscov2\_multicase/weigang\_raw\_feature\_protocol.md}
    \item \texttt{configs/phase2\_weigang\_raw\_feature\_protocol.yaml}
\end{itemize}

\textbf{Script added:}
\begin{itemize}
    \item \texttt{scripts/phase2\_sarscov2\_multicase/19\_freeze\_weigang\_raw\_feature\_protocol.py}
\end{itemize}

\textbf{Interpretation:} The Weigang raw branch is now protocol-defined but not downloaded. The primary trajectory remains restricted to nine swab samples. Isolate samples remain excluded from the primary branch. \(R_v\) remains unavailable until a clinical proxy such as Ct, viral load, culture status, or endpoint is recovered.

\textbf{Failure guard:} No raw sequencing files should be downloaded or interpreted until the frozen protocol audit passes. Sequencing depth remains a technical variable, not a biological HRSM axis.

\section*{Entry 040: Weigang controlled FASTQ download script added}

\textbf{Date:} \today

\textbf{Purpose:} To add a controlled, resumable raw FASTQ download script for the mapped Weigang primary swab trajectory.

\textbf{Script added:}
\begin{itemize}
    \item \texttt{scripts/phase2\_sarscov2\_multicase/20\_download\_weigang\_primary\_swab\_fastqs.py}
\end{itemize}

\textbf{Interpretation:} The script is gated by the frozen raw-feature protocol audit and downloads only the 18 planned submitted FASTQ files for the nine primary swab timepoints.

\textbf{Failure guard:} This script should be run only when ready to download raw sequencing files. It is resumable and records file sizes and SHA256 hashes after download.


\section*{Entry 041: Weigang primary swab FASTQ files downloaded}

\textbf{Date:} \today

\textbf{Purpose:} To download the raw FASTQ files for the mapped Weigang primary swab trajectory after the raw-feature protocol audit passed.

\textbf{Script executed:}
\begin{itemize}
    \item \texttt{scripts/phase2\_sarscov2\_multicase/20\_download\_weigang\_primary\_swab\_fastqs.py}
\end{itemize}
...
\textbf{Result:}
\begin{itemize}
    \item Planned files: \texttt{18}
    \item Downloaded or existing files: \texttt{18}
    \item Failed files: \texttt{0}
    \item Total bytes: \texttt{1128450798}
    \item Download log: \texttt{metadata/phase2\_sarscov2\_multicase/weigang\_primary\_swab\_fastq\_download\_log.csv}
\end{itemize}

\textbf{Interpretation:} The Weigang raw branch is now locally staged for the nine primary swab timepoints. Each downloaded FASTQ file has a SHA256 checksum recorded in the download log.

\textbf{Failure guard:} Raw FASTQ files must not be committed to Git. Only the download log, checksums, scripts, and derived lightweight metadata should be tracked.
